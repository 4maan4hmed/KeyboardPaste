function visualize_ble_stage()
    ...
    % 3. Packetization and encode with Hamming code
...
function encoded_data = custom_hamming_encode(data)
    ...
    for i = 1:size(data_blocks, 1)
        block = data_blocks(i, :);
        encoded_data(i, :) = [block(1) block(2) block(3) block(4) ...
                                mod(sum(block([1 2 4])), 2) ...
                                mod(sum(block([1 3 4])), 2) ...
                                mod(sum(block([2 3 4])), 2)];
    end
...
function whitened_data = custom_data_whitening(data)
    ...
    lfsr = [1 0 1 1 0 0 1];  % 7-bit LFSR initial state (a common choice)

    for i = 1:length(data)
        whitening_seq(i) = lfsr(end);
        new_bit = mod(sum(lfsr .* [1 0 1 1 1 0 0]), 2);  % LFSR feedback polynomial
